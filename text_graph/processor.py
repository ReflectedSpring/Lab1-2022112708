...
class TextProcessor:
    ...
    def calc_pagerank(graph: TextGraph, damping: float = 0.85, iterations: int = 100) -> Dict[str, float]:
        """改进的PageRank算法，悬挂节点PR值均分给其他节点"""
        nodes = list(graph.nodes)
        if not nodes:
            return {}

        n = len(nodes)
        pr = {node: 1.0 / n for node in nodes}  # 初始PR值平均分布
        ...
        for _ in range(iterations):
            ...
            for node in nodes:
                ...
                # 添加悬挂节点贡献（均分给其他n-1个节点）
                if n > 1 and dangling_sum > 0:
                    rank += damping * dangling_sum / (n - 1)
    ...
    def draw_graph(
            graph: TextGraph,
            output_path: str = 'text_graph.png',
            layout: str = 'spring',
            node_size: int = 800,
